...
from graph_viz import graph_to_mermaid, plan_to_mermaid, save_mermaid
from persistence import SessionStore
from node_trace import print_node_trace
...
class Graph:
    ...
    def extend_from(self, src_nid: str, result: AgentResult,
        ...
        # internal_successors are hidden pipeline steps of src_nid. Any other
        # pending node that already depends on src_nid must also wait for
        # those steps — otherwise it can become ready as soon as src_nid
        # completes and race ahead (e.g. formatter seeing coder `code` before
        # sandbox `stdout` exists).
        if internal_ids:
            for dep_nid, data in self.g.nodes(data=True):
                if dep_nid in internal_ids:
                    continue
                if data.get("status") != "pending":
                    continue
                ins = list(data.get("inputs") or [])
                preds = list(self.g.predecessors(dep_nid))
                if src_nid not in ins and src_nid not in preds:
                    continue
                for int_id in internal_ids:
                    if int_id not in ins:
                        ins.append(int_id)
                        self.g.add_edge(int_id, dep_nid)
                self.g.nodes[dep_nid]["inputs"] = ins

        # Critic auto-insertion: guard every outgoing edge from critic:true
        # skills — children added here and pre-planned pending successors.
        if src_def.critic:
            children_to_guard: list[str] = list(added)
            for child_nid in self.g.successors(src_nid):
                if child_nid in children_to_guard:
                    continue
                if self.g.nodes[child_nid].get("status") != "pending":
                    continue
                ...
                if any(
                    ...
                    continue
                children_to_guard.append(child_nid)

            for child_nid in children_to_guard:
...
class Executor:
    ...
    async def run(self, query: str, *, session_id: str | None = None,
                  resume: bool = False, show_graph: bool = False) -> str:
        ...
        if resume:
            ...
            if existing is None:
                raise RuntimeError(
                    f"cannot resume {sid}: no graph.json (or legacy graph.pkl) on disk"
                )
            graph_obj = existing
        ...
        while True:
            ...
            for nid, result, prompt in outcomes:
                ...
                if result.success:
                    ...
                    if show_graph and graph.g.nodes[nid]["skill"] == "planner":
                        plan_md = plan_to_mermaid(
                            result.successors, parent=nid, title="planner intent",
                        )
                        expanded_md = graph_to_mermaid(
                            graph.g, title="orchestrator expanded",
                        )
                        print(f"\n[PLAN]\n{plan_md}\n")
                        print(f"[EXPANDED]\n{expanded_md}\n")
                        save_mermaid(plan_md, store.dir / "plan.mmd")
                        save_mermaid(expanded_md, store.dir / "expanded.mmd")
        ...
        if critic_fail_cap_hit:
            ...
            print(f"\n[flow] WARNING: critic-fail cap hit on "
                  ...
                  f"output too.")
        if show_graph:
            executed_md = graph_to_mermaid(graph.g, title="executed")
            print(f"\n[EXECUTED]\n{executed_md}\n")
            save_mermaid(executed_md, store.dir / "executed.mmd")
...
def _parse_cli(argv: list[str]) -> tuple[str, str | None, bool, bool]:
    show_graph = False
    ...
    while i < len(argv):
        ...
        if arg == "--show-graph":
            ...
            i += 1
    query = " ".join(positional) or "Say hello in one short sentence."
    return query, resume_sid, bool(resume_sid), show_graph


def main() -> None:
    query, resume_sid, resume, show_graph = _parse_cli(sys.argv[1:])
    asyncio.run(Executor().run(
        query, session_id=resume_sid, resume=resume, show_graph=show_graph,
    ))
